...
class classy(_cosmo):
    ...
    def needs(self, **requirements):
        ...
        for k, v in self._needs.items():
            ...
            #EDE-edit: adding fsigma8 for RSD. changing it to take an argument
            elif k == "fsigma8":
                self.collectors[k] = collector(
                    method="get_fsigma8",
                    args=[np.atleast_1d(v["z"])],
    ...
    def compute(self, _derived=None, cached=True, **params_values_dict):
        ...
        try:
            ...
            # EDE-edit: added z_c and fEDE error codes.
            except CosmoComputationError as e:
                if ("zc" in str(e) or
                    "fEDE" in str(e) or
                    "You have asked for an unrealistic high value omega_b" in str(e) or
                ...
                return 0
#EDE-edit for Pk exit code
            try:
                self.classy.get_pk_and_k_and_z()
            # "Valid" failure of CLASS: parameters too extreme -> log and report
            # EDE-edit
            except CosmoComputationError as e:
                if ("encompass" in str(e)):
                    pass
                else:
                    self.log.debug("Computation of cosmological products failed. "
                                   "Assigning 0 likelihood and going on. "
